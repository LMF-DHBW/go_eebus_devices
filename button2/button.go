...
func main() {
	...
	eebusNode := eebus.NewEebusNode("100.90.1.99", false, "button2", "0002", "DHBW", "Button")
...
func buildDeviceModel(eebusNode *eebus.EebusNode) {
	...
	eebusNode.DeviceStructure.DeviceAddress = "Switch 2"
	eebusNode.DeviceStructure.Entities = []*resources.EntityModel{
		{
			...
			Features: []*resources.FeatureModel{
				eebusNode.DeviceStructure.CreateNodeManagement(false),
				{
					...
					BindingTo:        []string{"ActuatorSwitch2"},
					MaxSubscriptions: 3,
					MaxBindings:      1,
				},
			},
